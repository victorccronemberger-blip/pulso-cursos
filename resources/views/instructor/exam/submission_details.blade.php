...
<div class="ol-card">
    <div class="ol-card-body">
        <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
            ...
            <a href="{{ route('instructor.exam.submissions', $exam->id) }}" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
...
<div class="ol-card mb-4">
    <div class="ol-card-body py-3 px-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h4 class="title fs-16px mb-1 fw-semibold">
                    {{ optional($submission->exam)->title ?? 'N/A' }}
                </h4>
            </div>
            @if($submission->status === 'published')
            <span class="badge bg-primary px-3 py-2">{{ get_phrase('Published') }}</span>
    ...
    <div class="col-lg-4">
        ...
        <div class="ol-card">
            <div class="ol-card-body p-4">
                ...
                <form action="{{ route('instructor.submission.update', $submission->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
...
<script>
    ...
    function setupDrawing(canvas, pageNum) {
        ...
        function handleMove(e) {
            ...
            if (currentTool === 'pen') {
                ...
                const pressure = Math.pow(1 - Math.min(speed / 2, 1), 2);
                ...
                if (penPoints.length >= 4) {
                    const p0 = penPoints[penPoints.length - 4],
                        p1 = penPoints[penPoints.length - 3];
                    const p2 = penPoints[penPoints.length - 2],
                        p3 = penPoints[penPoints.length - 1];
                    ...
                    for (let i = 0; i < segments; i++) {
                        ...
                        ctx.lineWidth = p1.size + (p2.size - p1.size) * t;
                        ctx.beginPath();
                        ctx.moveTo(x1, y1);
                        ctx.lineTo(x2, y2);
                        ctx.stroke();
    ...
    function drawSmoothLine(ctx, x1, y1, x2, y2, cp1x, cp1y, cp2x, cp2y, color, size1, size2) {
        ctx.save();
        ctx.strokeStyle = color;
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        if (cp1x !== undefined) {
            ...
            for (let i = 0; i < segments; i++) {
                ...
                ctx.lineWidth = size1 + (size2 - size1) * t;
                ctx.beginPath();
                ctx.moveTo(bx1, by1);
                ctx.lineTo(bx2, by2);
                ctx.stroke();
    ...
    function eraseAtPoint(canvas, pageNum, x, y) {
        ...
        drawings[pageNum] = drawings[pageNum].filter(item => {
            if (item.tool === 'text') return Math.sqrt(Math.pow(item.x - x, 2) + Math.pow(item.y - y, 2)) > eraserSize;
            if (item.x1 !== undefined) return pointToLineDistance(x, y, item.x1, item.y1, item.x2, item.y2) > eraserSize;
            return true;
        });
    ...
    function findObjectAt(pageNum, x, y) {
        for (let i = drawings[pageNum].length - 1; i >= 0; i--) {
            if (isPointInObject(drawings[pageNum][i], x, y)) return drawings[pageNum][i];
        }
        return null;
    }
    ...
    function isPointInObject(obj, x, y) {
        const tolerance = 10;
        if (obj.tool === 'text') {
            const tw = obj.text.length * obj.size * 0.6;
            return x >= obj.x - tolerance && x <= obj.x + tw + tolerance && y >= obj.y - obj.size - tolerance && y <= obj.y + tolerance;
        }
        if (obj.tool === 'rectangle') {
            return x >= Math.min(obj.x1, obj.x2) - tolerance && x <= Math.max(obj.x1, obj.x2) + tolerance && y >= Math.min(obj.y1, obj.y2) - tolerance && y <= Math.max(obj.y1, obj.y2) + tolerance;
        }
        if (obj.tool === 'circle') {
            const r = Math.sqrt(Math.pow(obj.x2 - obj.x1, 2) + Math.pow(obj.y2 - obj.y1, 2));
            return Math.abs(Math.sqrt(Math.pow(x - obj.x1, 2) + Math.pow(y - obj.y1, 2)) - r) <= tolerance;
        }
    ...
    function getObjectBounds(obj) {
        ...
        return {
            x: Math.min(obj.x1, obj.x2) - 10,
            y: Math.min(obj.y1, obj.y2) - 10,
            width: Math.abs(obj.x2 - obj.x1) + 20,
            height: Math.abs(obj.y2 - obj.y1) + 20
        };
    }
    ...
    function getResizeHandleAt(wrapper, x, y) {
        ...
        for (let handle of wrapper.querySelectorAll('.resize-handle')) {
            ...
            const hx = rect.left - wr.left + rect.width / 2,
                hy = rect.top - wr.top + rect.height / 2;
    ...
    function resizeObject(canvas, pageNum, x, y) {
        ...
        if (selectedObject.tool === 'text') {
            selectedObject.size = Math.max(8, Math.min(72, Math.sqrt(Math.pow(x - selectedObject.x, 2) + Math.pow(y - selectedObject.y, 2)) / 2));
    ...
    document.querySelectorAll('[data-tool]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.querySelectorAll('.drawing-canvas').forEach(c => updateCanvasCursor(c));
            document.getElementById('fontSizeControl').style.display = currentTool === 'text' ? 'flex' : 'none';
    ...
    document.getElementById('saveBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            const response = await fetch("{{ route('instructor.submission.annotated.upload', $submission->id) }}", {
                method: 'POST',
                body: formData,
                headers: {
