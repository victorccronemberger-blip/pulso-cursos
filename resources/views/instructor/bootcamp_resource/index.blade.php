<form action="{{ route('instructor.bootcamp.resource.store') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="fpb-7 mb-3">
        ...
        <select class="neu-select " name="upload_type" id="upload_type" required>
        ...
        @foreach ($files as $file)
            <li class="file">
                ...
                <div class="d-flex align-items-center gap-2 ms-auto action-btns">
                    <a href="{{ route('instructor.bootcamp.resource.download', $file->id) }}" data-bs-toggle="tooltip" title="{{ get_phrase('Download') }}" data-bs-placement="bottom"><i class="fi fi-rr-down-to-line"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" onclick="confirmModal('{{ route('instructor.bootcamp.resource.delete', $file->id) }}')" data-bs-toggle="tooltip" title="{{ get_phrase('Delete') }}" data-bs-placement="bottom"><span class="fi-rr-trash"></span></a>
                </div>
            </li>
        @endforeach
    </ul>
