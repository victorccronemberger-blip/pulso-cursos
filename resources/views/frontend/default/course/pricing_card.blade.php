<div class="gradient-border radius-22 page-static-sidebar">
    <div class="ps-box ps-sidebar">
        ...
        <div class="ps-price mb-3">
            @if (isset($course_details->is_paid) && $course_details->is_paid == 0)
            <h4 class="g-title-dark free-badge">{{ get_phrase('Free') }}</h4>

            @elseif (isset($course_details->discount_flag) && $course_details->discount_flag == 1)
            <div class="d-flex align-items-baseline gap-2">
                <h2 class="g-title-dark mb-0">{{ currency($course_details->discounted_price, 2) }}</h2>
                <del class="text-muted small">{{ currency($course_details->price, 2) }}</del>
            </div>

            @else
            <h2 class="g-title-dark mb-0">{{ currency($course_details->price, 2) }}</h2>
            @endif
        </div>
        ...
        @if ($in_wishlist)
        <span class="eBtn gradient w-100 cursor-pointer mt-3 toggleWishItem" onclick="wishlistToggleButton('{{ $course_details->id }}', this)">
            {{ get_phrase('Remove from wishlist') }}
        </span>
        @else
        <span class="eBtn learn-btn w-100 cursor-pointer mt-3 toggleWishItem mb-0" onclick="wishlistToggleButton('{{ $course_details->id }}', this)">
