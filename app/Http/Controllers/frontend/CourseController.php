...
class CourseController extends Controller
{
    ...
    public function freecourses(Request $request, $category = '')
    {
        $layout = Session::has('view') ? session('view') : 'grid';
        $page_data['layout'] = $layout;
        ...
        if ($category != '') {
            ...
            if ($category_details) {
                if ($category_details->parent_id > 0) {
                    ...
                    $query->where('courses.category_id', $category_details->id);
                } else {
                    $sub_cat_id = Category::where('parent_id', $category_details->id)
                        ->pluck('id')
                        ->toArray();

                    $sub_cat_id[] = $category_details->id;

                    $query->whereIn('courses.category_id', $sub_cat_id);
        ...
        if ($request->has('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('courses.title', 'LIKE', "%{$search}%")
                    ...
                    ->orWhere('courses.meta_keywords', 'LIKE', "%{$search}%")
                    ->orWhere('courses.meta_description', 'LIKE', "%{$search}%")
        ...
        // filter by level
        if ($request->has('level')) {
            $query->where('courses.level', $request->query('level'));
        }

        // filter by language
        if ($request->has('language')) {
            $query->where('courses.language', $request->query('language'));
        }

        // filter by rating
        if ($request->has('rating')) {
            $query->where('courses.average_rating', $request->query('rating'));
        }
        ...
        $page_data['courses'] = $query->latest('courses.id')
            ->paginate($layout == 'grid' ? 9 : 5)
        ...
        return view(
            'frontend.' . get_frontend_settings('theme') . '.course.free_course',
            $page_data
        );
    }
}
