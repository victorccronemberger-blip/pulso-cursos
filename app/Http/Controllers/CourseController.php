...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'preview'             => 'nullable|mimes:mp4,mov,avi,webm|max:102400',
        ]);
        ...
        $data['instructor_ids'] = json_encode(array_values(array_map('intval', $request->instructors)));
        $data['created_at']     = date('Y-m-d H:i:s');
        $data['updated_at']     = date('Y-m-d H:i:s');

        // ✅ SECURITY: File types already validated above via $request->validate()
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->tab == 'basic') {
            ...
            $existing             = array_map('intval', json_decode($course_instructor_id->instructor_ids, true) ?? []);
            $new                  = array_map('intval', $request->instructors ?? []);
            $merged               = array_unique(array_merge($existing, $new));
            $data['instructor_ids'] = json_encode(array_values($merged));
        ...
        } elseif ($request->tab == 'feature') {
            $features = array_filter($request->features ?? [], fn($value) => !is_null($value) && $value !== '');

            $validator = Validator::make(['features' => $features], [
                'features'   => 'array',
            ...
            // ✅ SECURITY: Validate all media file uploads
            $rules = [
                'thumbnail' => 'nullable|image|mimes:jpg,jpeg,png,webp,gif|max:2048',
                'banner'    => 'nullable|image|mimes:jpg,jpeg,png,webp,gif|max:4096',
                'preview'   => 'nullable|mimes:mp4,mov,avi,webm|max:102400',
            ...
            // ✅ SECURITY: Validate og_image file
            if ($request->og_image) {
                $request->validate([
                    'og_image' => 'image|mimes:jpg,jpeg,png,webp,gif|max:2048',
                ]);
            }
            ...
            if ($request->og_image) {
                // ✅ SECURITY: Never use getClientOriginalName() — use uniqid + extension only
                $safeFileName    = $course_details->id . '-' . uniqid() . '.' . $request->og_image->extension();
                $destinationPath = 'uploads/seo-og-images/' . $safeFileName;
