...
class SettingController extends Controller
{
    ...
    public function website_settings_update(Request $request)
    {
        ...
        if ($request->type == 'banner_video') {
            array_shift($data);

            if (isset($request->banner_video) && $request->banner_video != '') {

                // Remove old video file to save disk space
                $existing = get_frontend_settings('banner_video');
                if ($existing && file_exists(public_path($existing))) {
                    @unlink(public_path($existing));
                }

                $extension = $request->banner_video->extension();

                $videoPath = "uploads/banner_video/" . nice_file_name('banner_video', $extension);

                FileUploader::upload($request->banner_video, $videoPath);

                FrontendSetting::where('key', 'banner_video')->update(['value' => $videoPath]);

                Session::flash('success', get_phrase('Banner video updated successfully'));
            }
        }
    ...
    function phrase_import($lan_id)
    {
        ...
        foreach (Language_phrase::where('language_id', $english_lan_id)->get() as $en_lan_phrase) {
            if (Language_phrase::where('language_id', $lan_id)->where('phrase', $en_lan_phrase->phrase)->count() == 0) {
                Language_phrase::insert(['language_id' => $lan_id, 'phrase' => $en_lan_phrase->phrase, 'translated' => $en_lan_phrase->translated, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => now()]);
            }
        }
    ...
    public function language_store(Request $request)
    {
        ...
        if (Language::where('name', 'like', $request->language)->count() == 0) {
            ...
            foreach (Language_phrase::where('language_id', $english_lan_id)->get() as $en_lan_phrase) {
                Language_phrase::insert(['language_id' => $new_lan_id, 'phrase' => $en_lan_phrase->phrase, 'translated' => $en_lan_phrase->translated, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => now()]);
            }
    ...
    function certificate_update_template(Request $request)
    {
        ...
        if ($row->count() > 0) {
            ...
            $path = FileUploader::upload($request->certificate_template, 'uploads/certificate-template', 1000);
            Setting::insert(['type' => 'certificate_template', 'description' => $path, 'updated_at' => now(), 'created_at' => now()]);
        }
    ...
    function certificate_builder_update(Request $request)
    {
        ...
        if ($row->count() > 0) {
            ...
            Setting::insert(['type' => 'certificate_builder_content', 'description' => $request->certificate_builder_content, 'updated_at' => now(), 'created_at' => now()]);
        }
        ...
        if ($request->type == 'watermark') {
            ...
            foreach ($watermark as $key => $data) {
                if (!PlayerSettings::where('title', $key)->exists()) {
                    PlayerSettings::insert(['title' => $key, 'description' => $data, 'updated_at' => now(), 'created_at' => now()]);
                    continue;
                }
