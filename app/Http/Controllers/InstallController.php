...
class InstallController extends Controller
{
    ...
    {
        // If database is configured and has users, skip installer
        try {
            if (DB::connection()->getDatabaseName() !== 'db_name' && DB::table('users')->count() > 0) {
                return redirect('/login');
            }
        } catch (\Exception $e) {
            // Table doesn't exist yet, continue to installer
        }
    ...
    public function configure_routes()
        {
            // No longer modifying routes/web.php — installation is detected
